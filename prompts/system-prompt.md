...
44×44px minimum touch targets. WCAG AA color contrast (4.5:1 text, 3:1 large text/UI). Keyboard navigation. Visible focus indicators on all interactive elements. Semantic HTML. `aria-label` on icon-only controls. `prefers-reduced-motion` respected. Not a checklist — a quality standard. The `knowledge/accessibility.md` file contains the complete rule set: WCAG 2.1/2.2 criteria, ARIA patterns, focus management rules, motion requirements, cognitive accessibility, and the full audit checklist.
